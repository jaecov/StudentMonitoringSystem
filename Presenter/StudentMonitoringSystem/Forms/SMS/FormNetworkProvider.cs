...
{
    public partial class FormNetworkProvider : WeifenLuo.WinFormsUI.Docking.DockContent, INetworkProvider
    {
        #region Constructor

        public FormNetworkProvider()
        {           
            InitializeComponent();
            grdNetworkProvider.AutoGenerateColumns = false;
            Presenter = new NetworkProviderPresenter(this);
            Name = string.Empty;
        }

        public NetworkProviderPresenter Presenter
        {
            get;
            set;
        }
        ...
        private void FormNetworkProvider_Load(object sender, EventArgs e)
        {
            Presenter.LoadItems();
        }

        private void grdNetworkProvider_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex <= -1)
                return;

            if (grdNetworkProvider.CurrentRow == null)
                return;

            var data = grdNetworkProvider.CurrentRow.DataBoundItem as sms_networkprovider;
            if (data == null)
                return;

            int id = data.id;
            Presenter.LoadNetworkProviderInfo(id);
        }
        ...
        public List<sms_networkprovider> NetworkProviderDataSource
        {
            set
            {
                NetworkProviderinfoBindingSource.DataSource = value;
            }
        }
